...
abstract class FileValue {
  ...
  FileValue({@required this.file}) : assert(file != null);
}
...
class PersistentFileItem extends FileValue {
  ...
  PersistentFileItem({@required ReadableFile file, this.hasDeleteMark = false})
      : assert(hasDeleteMark != null),
        super(file: file);
...
class ReadableFileView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    } else if (file is LocalReadableFile) {
...
class FileValueMenuButton extends StatelessWidget {
  ...
  const FileValueMenuButton({Key key, @required this.value, @required this.child})
      : assert(value != null),
        assert(child != null),
        super(key: key);
